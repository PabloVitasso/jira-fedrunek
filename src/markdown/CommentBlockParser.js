...
export function parseCommentBlocks(content) {
  console.log(
    '[parseCommentBlocks] step 1: regex-matching <!-- comment_id: N --> blocks in existing file content'
  );
  const blocks = {};
  ...
  console.log(
    `[parseCommentBlocks] step 3: indexed blocks by commentId, keys: ${Object.keys(blocks)}`
  );
  return blocks;
}
...
export function mergeComments(existingBlocks, freshComments, downloadedAt) {
  console.log(
    `[mergeComments] step 1: diffing ${freshComments.length} fresh comments against ${Object.keys(existingBlocks).length} existing blocks`
  );
  const seen = new Set();
  const result = [];
  ...
  for (const comment of freshComments) {
    ...
    if (!existing) {
      console.log(
        `[mergeComments] step 2: comment_id=${comment.id} is new -> appending formatted block`
      );
      result.push(formatComment(comment, downloadedAt));
    } else if (existing.updatedAt !== comment.updated) {
      console.log(
        `[mergeComments] step 3: comment_id=${comment.id} changed (${existing.updatedAt} -> ${comment.updated}) -> replacing block`
      );
      result.push(formatComment(comment, downloadedAt));
    } else {
      console.log(
        `[mergeComments] step 4: comment_id=${comment.id} unchanged -> keeping existing block as-is`
      );
  ...
  for (const [id, existing] of Object.entries(existingBlocks)) {
    ...
    if (existing.block.includes('<!-- deleted_at:')) {
      console.log(
        `[mergeComments] step 5: comment_id=${id} already marked deleted -> keeping existing block as-is`
      );
      result.push(existing.block);
      continue;
    }
    console.log(
      `[mergeComments] step 5: comment_id=${id} missing from fresh comments -> marking deleted_at=${downloadedAt}`
    );
