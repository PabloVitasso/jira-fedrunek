...
export class ConfluenceSyncEngine {
  ...
  async #writePage(page, dirPart) {
    console.log(`[ConfluenceSyncEngine.#writePage] step 1: fetching content for page ${page.id} (${page.title})`);
    ...
    console.log(`[ConfluenceSyncEngine.#writePage] step 2: writing ${filePath} via FileWriter.write`);
    this.fileWriter.write(filePath, content);
    this.syncState.setPage(page.id, { lastModified: page.lastModified, path: rel, title: page.title });
    return { id: page.id, path: rel, title: page.title };
  }

  async #fetchStale(pages, dirPart) {
    const stale = pages.filter((p) => (this.syncState.getPage(p.id)?.lastModified ?? '') !== p.lastModified);
    console.log(`[ConfluenceSyncEngine.#fetchStale] step 1: ${pages.length} pages, ${stale.length} stale, ${pages.length - stale.length} up-to-date`);
    if (stale.length > 1) {
      const proceed = await this.confirmBulk(stale);
      if (!proceed) {
        console.log('[ConfluenceSyncEngine.#fetchStale] step 2: bulk download not confirmed, aborting');
        return { fetched: 0, failed: [], skipped: pages.length - stale.length, aborted: true };
      }
    }
    const limiter = pLimit(this.concurrency);
    const results = await Promise.allSettled(stale.map((p) => limiter(() => this.#writePage(p, dirPart ?? p.spaceKey))));
    const failed = results.filter((r) => r.status === 'rejected');
    this.syncState.save();
    console.log(`[ConfluenceSyncEngine.#fetchStale] step 3: ${stale.length - failed.length} fetched, ${failed.length} failed`);
    return { fetched: stale.length - failed.length, failed, skipped: pages.length - stale.length, aborted: false };
  }

  async #rebuildContents() {
    console.log('[ConfluenceSyncEngine.#rebuildContents] step 1: rebuilding CONTENTS.md from SyncState confluence entries');
    const entries = this.syncState.allPages().map(([id, meta]) => ({ id, ...meta }));
    const lines = ['# Confluence Cache', `> synced: ${this.now().slice(0, 10)}, ${entries.length} pages`];
    ...
    for (const space of Object.keys(bySpace).sort()) {
      ...
      const byFolder = groupBy(spaceEntries.filter((e) => parts(e).length >= 3), (e) => parts(e)[1]);
      topLevel.sort((a, b) => a.title.localeCompare(b.title)).forEach((e) => lines.push(`- [${e.title}](${linkPath(e)})`));
      for (const folder of Object.keys(byFolder).sort()) {
        lines.push('', `### ${folder}`);
        byFolder[folder].sort((a, b) => a.title.localeCompare(b.title)).forEach((e) => lines.push(`  - [${e.title}](${linkPath(e)})`));
      }
    }
    this.fileWriter.write(path.join(this.outDir, 'CONTENTS.md'), lines.join('\n') + '\n');
    console.log(`[ConfluenceSyncEngine.#rebuildContents] step 2: CONTENTS.md written, ${entries.length} entries`);
  }
  ...
  #removeOrphans(remotePages, { underDir } = {}) {
    ...
    for (const [id, entry] of this.syncState.allPages()) {
      const underScope = underDirFwd ? entry.path.replace(/\\/g, '/').startsWith(`${underDirFwd}/`) : true;
      if (underScope && !remoteIds.has(id)) {
        console.log(`[ConfluenceSyncEngine.#removeOrphans] step 1: removing orphan page ${id} (${entry.title})`);
  ...
  async syncPage(id) {
    console.log(`[ConfluenceSyncEngine.syncPage] step 1: looking up page ${id} via CQL id = "${id}"`);
    const meta = (await this.confluenceClient.searchByCql(`id = "${id}"`, { limit: 1 })).results?.[0];
    if (!meta) {
    ...
    const page = {
      id,
      title: meta.content?.title ?? meta.title ?? id,
      spaceKey: meta.content?._links?.webui?.split('/')?.[2] ?? 'unknown',
      lastModified: meta.lastModified ?? '',
    };
    const stored = this.syncState.getPage(id);
    if (stored?.lastModified === page.lastModified) {
      console.log(`[ConfluenceSyncEngine.syncPage] step 3: unchanged (lastModified=${page.lastModified}), skipping`);
  ...
  async syncDir(folderId, label) {
    console.log(`[ConfluenceSyncEngine.syncDir] step 1: walking descendants of folder ${folderId} (${label})`);
  ...
  async syncDirs(watchDirs) {
    console.log(`[ConfluenceSyncEngine.syncDirs] step 1: syncing ${watchDirs.length} watched folder(s)`);
    const results = [];
  ...
  async syncPages(watchPages) {
    console.log(`[ConfluenceSyncEngine.syncPages] step 1: syncing ${watchPages.length} watched page(s)`);
    const results = [];
  ...
  async syncSpaces(spaceKeys) {
    console.log(`[ConfluenceSyncEngine.syncSpaces] step 1: resolving space keys ${JSON.stringify(spaceKeys)} to ids`);
    ...
    console.log('[ConfluenceSyncEngine.syncSpaces] step 2: listing all pages in each space (cursor-paginated)');
    const allPages = [];
    ...
    console.log(`[ConfluenceSyncEngine.syncSpaces] step 3: ${allPages.length} pages total across ${spaceKeys.length} space(s)`);
