...
export class McpSession {
  ...
  async connect() {
    step(
      'cyan',
      `[McpSession.connect] step 1: constructing MCP client + transport for ${this.mcpUrl}`
    );
    ...
    step(
      'blue',
      `[McpSession.connect] step 2: connecting — browser may open, authorize within ${AUTH_TIMEOUT_S}s`
    );
    let timeoutId;
    try {
      await Promise.race([
        ...
        new Promise((_, reject) => {
          timeoutId = setTimeout(
            () =>
              reject(
                new Error(
                  `Authorization timed out after ${AUTH_TIMEOUT_S}s. Re-run and authorize promptly.`
                )
              ),
  ...
  async callTool(name, args) {
    ...
    const retryOpts = {
      ...
      onFailedAttempt: async err => {
        const delay =
          Math.min(RETRY_CAP_MS, RETRY_BASE_MS * 2 ** err.attemptNumber) + Math.random() * 1_000;
        step(
          'yellow',
          `[McpSession.callTool] step 2: attempt ${err.attemptNumber} failed for ${name} — retrying in ${Math.round(delay)}ms`
        );
    ...
    const result = await pRetry(
      () =>
        this.client.callTool({ name, arguments: { cloudId: this.cloudId, ...args } }, undefined, {
  ...
  #cleanup() {
    if (this.cleanedUp) return Promise.resolve();
    this.cleanedUp = true;
    ...
    return this.client?.close().catch(() => {}) ?? Promise.resolve();
  }
