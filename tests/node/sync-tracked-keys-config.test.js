...
test('TC-SYNC-TRACKEDKEYS-001: load() returns an empty array when no config file exists', () => {
  console.log(
    '[TC-SYNC-TRACKEDKEYS-001] step 1: creating a TrackedKeysConfig pointed at a non-existent file'
  );
...
test('TC-SYNC-TRACKEDKEYS-002: load() parses tracked_keys from an existing TOML file', () => {
  ...
  console.log(
    '[TC-SYNC-TRACKEDKEYS-002] step 2: loading it via TrackedKeysConfig and asserting the parsed array'
  );
...
test('TC-SYNC-TRACKEDKEYS-003: add() creates the file and persists a new tracked key', () => {
  console.log(
    '[TC-SYNC-TRACKEDKEYS-003] step 1: creating a TrackedKeysConfig pointed at a non-existent file'
  );
  const configPath = makeTempPath();
  const config = new TrackedKeysConfig(configPath);
  ...
  console.log(
    `[TC-SYNC-TRACKEDKEYS-003] step 3: asserting returned list and round-trip via a fresh instance, got: ${JSON.stringify(result)}`
  );
...
test('TC-SYNC-TRACKEDKEYS-004: add() appends to existing tracked_keys without dropping prior entries', () => {
  ...
  console.log(
    `[TC-SYNC-TRACKEDKEYS-004] step 3: asserting both keys present, got: ${JSON.stringify(result)}`
  );
...
test('TC-SYNC-TRACKEDKEYS-006: add() preserves unrelated top-level keys already in the TOML file', () => {
  console.log(
    '[TC-SYNC-TRACKEDKEYS-006] step 1: writing a TOML file with cloud_id and a confluence table alongside tracked_keys'
  );
  const configPath = makeTempPath();
  fs.writeFileSync(
    configPath,
  ...
  config.add(['PROJ-2']);
  console.log(
    '[TC-SYNC-TRACKEDKEYS-006] step 3: re-parsing the file and asserting cloud_id/confluence survived'
  );
...
test('TC-SYNC-TRACKEDKEYS-005: add() is idempotent for an already-tracked key', () => {
  ...
  const result = config.add(['PROJ-1']);
  console.log(
    `[TC-SYNC-TRACKEDKEYS-005] step 3: asserting no duplicate entry, got: ${JSON.stringify(result)}`
  );
  assert.deepEqual(result, ['PROJ-1']);
});
...
test('TC-SYNC-TRACKEDKEYS-007: load() returns an empty array when the file exists but has no tracked_keys key', () => {
  console.log(
    '[TC-SYNC-TRACKEDKEYS-007] step 1: writing a TOML file with cloud_id only, no tracked_keys'
  );
  const configPath = makeTempPath();
...
test('TC-SYNC-TRACKEDKEYS-008: add() with a mix of new and already-tracked keys only appends the new ones', () => {
  ...
  console.log(
    `[TC-SYNC-TRACKEDKEYS-008] step 3: asserting PROJ-1 not duplicated and PROJ-3 appended, got: ${JSON.stringify(result)}`
  );
  assert.deepEqual(result, ['PROJ-1', 'PROJ-3']);
});

test('TC-SYNC-TRACKEDKEYS-010: load(data) uses the supplied pre-parsed TOML instead of re-reading the file', () => {
  console.log(
    '[TC-SYNC-TRACKEDKEYS-010] step 1: writing a TOML file with tracked_keys=["ON-DISK"] but constructing load() with a different pre-parsed object'
  );
  const configPath = makeTempPath();
  fs.writeFileSync(configPath, '[jira]\ntracked_keys = ["ON-DISK"]\n', 'utf8');
  const config = new TrackedKeysConfig(configPath);
  console.log(
    '[TC-SYNC-TRACKEDKEYS-010] step 2: calling load({ jira: { tracked_keys: ["PRE-PARSED"] } })'
  );
  const result = config.load({ jira: { tracked_keys: ['PRE-PARSED'] } });
  console.log(
    `[TC-SYNC-TRACKEDKEYS-010] step 3: asserting the pre-parsed data was used, not a fresh disk read, got: ${JSON.stringify(result)}`
  );
  assert.deepEqual(result, ['PRE-PARSED']);
});

test('TC-SYNC-TRACKEDKEYS-009: add() does not double-append a key repeated within the same call', () => {
  console.log(
    '[TC-SYNC-TRACKEDKEYS-009] step 1: creating a TrackedKeysConfig pointed at a non-existent file'
  );
  const configPath = makeTempPath();
  const config = new TrackedKeysConfig(configPath);
  ...
  console.log(
    `[TC-SYNC-TRACKEDKEYS-009] step 3: asserting only one entry, got: ${JSON.stringify(result)}`
  );
  assert.deepEqual(result, ['PROJ-1']);
});
