...
function makeSyncState(initial = {}) {
  ...
  return {
    ...
    setIssue: (key, meta) => { state[key] = meta; },
    save: () => {},
    _state: state,
  };
}
...
test('TC-SYNC-ENGINE-004: syncAll calls syncIssue for every key and collects results in order', async () => {
  ...
  const issuesByKey = {
    'PROJ-1': makeIssue({ key: 'PROJ-1', fields: { ...makeIssue().fields, comment: { comments: [] } } }),
    'PROJ-2': makeIssue({ key: 'PROJ-2', fields: { ...makeIssue().fields, comment: { comments: [] } } }),
  };
  const jiraClient = {
...
test('TC-SYNC-ENGINE-005: syncIssue treats a missing fields.comment as zero comments', async () => {
  console.log('[TC-SYNC-ENGINE-005] step 1: wiring a fake JiraClient for an issue with no comment field at all');
  ...
  const result = await engine.syncIssue('PROJ-1');
  console.log(`[TC-SYNC-ENGINE-005] step 3: asserting it still succeeds with zero comment_ids, got: ${JSON.stringify(syncState._state['PROJ-1'])}`);
  assert.deepEqual(result, { status: 'created', key: 'PROJ-1' });
