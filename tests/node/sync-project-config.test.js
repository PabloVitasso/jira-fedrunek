...
test('TC-PROJECT-CONFIG-001: load() on a missing file returns empty defaults', () => {
  console.log('[TC-PROJECT-CONFIG-001] step 1: creating a ProjectConfig pointed at a non-existent file');
  ...
  assert.deepEqual(result, { cloudId: undefined, confluence: { spaceKeys: [], watchPages: [], watchDirs: [] } });
});
...
test('TC-PROJECT-CONFIG-004: load() defaults confluence targets when [confluence] table is absent entirely', () => {
  console.log('[TC-PROJECT-CONFIG-004] step 1: writing a toml file with cloud_id only, no [confluence] table');
  const filePath = makeTempPath('cloud_id = "cloud-1"\n');
  console.log('[TC-PROJECT-CONFIG-004] step 2: calling load() and asserting confluence defaults to empty arrays');
  const result = new ProjectConfig(filePath).load();
...
test('TC-PROJECT-CONFIG-005: load() defaults confluence targets when [confluence] table is present but empty', () => {
  console.log('[TC-PROJECT-CONFIG-005] step 1: writing a toml file with an empty [confluence] table');
  const filePath = makeTempPath('cloud_id = "cloud-1"\n\n[confluence]\n');
  console.log('[TC-PROJECT-CONFIG-005] step 2: calling load() and asserting confluence defaults to empty arrays');
  const result = new ProjectConfig(filePath).load();
  assert.deepEqual(result.confluence, { spaceKeys: [], watchPages: [], watchDirs: [] });
});

test('TC-PROJECT-CONFIG-006: load() maps multiple watch_pages and watch_dirs entries in order', () => {
  console.log('[TC-PROJECT-CONFIG-006] step 1: writing a toml file with two watch_pages and two watch_dirs entries');
...
`);
  console.log('[TC-PROJECT-CONFIG-006] step 2: calling load() and asserting both lists preserve order');
  const result = new ProjectConfig(filePath).load();
...
test('TC-PROJECT-CONFIG-003: load() extracts [confluence] space_keys, watch_pages, watch_dirs', () => {
  console.log('[TC-PROJECT-CONFIG-003] step 1: writing a toml file with a full [confluence] section');
  const filePath = makeTempPath(`
cloud_id = "cloud-1"
...
label = "Interaction Inventory"
`);
  console.log('[TC-PROJECT-CONFIG-003] step 2: calling load() and asserting the confluence targets');
  const result = new ProjectConfig(filePath).load();
