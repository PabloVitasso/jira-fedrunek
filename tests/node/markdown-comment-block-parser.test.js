...
test('TC-MD-COMMENTPARSER-001: parseCommentBlocks extracts a single block keyed by id', () => {
  ...
  console.log(
    `[TC-MD-COMMENTPARSER-001] step 3: asserting map keyed by "10088", got keys: ${Object.keys(blocks)}`
  );
...
test('TC-MD-COMMENTPARSER-002: parseCommentBlocks extracts multiple blocks in order', () => {
  console.log(
    '[TC-MD-COMMENTPARSER-002] step 1: formatting two comments and concatenating their markdown'
  );
  ...
  const blocks = parseCommentBlocks(content);
  console.log(
    `[TC-MD-COMMENTPARSER-002] step 3: asserting both ids present, got keys: ${Object.keys(blocks)}`
  );
...
test('TC-MD-COMMENTPARSER-003: parseCommentBlocks returns an empty object for content with no comment blocks', () => {
  console.log(
    '[TC-MD-COMMENTPARSER-003] step 1: parsing plain content with no HTML comment markers'
  );
  const blocks = parseCommentBlocks('# Just an issue body\n\nNo comments here.');
  console.log(
    `[TC-MD-COMMENTPARSER-003] step 2: asserting empty map, got: ${JSON.stringify(blocks)}`
  );
...
test('TC-MD-COMMENTPARSER-004: mergeComments appends a block for a new comment id', () => {
  console.log(
    '[TC-MD-COMMENTPARSER-004] step 1: merging one fresh comment against an empty existing-blocks map'
  );
  const result = mergeComments({}, [makeComment()], DOWNLOADED_AT);
  console.log(
    `[TC-MD-COMMENTPARSER-004] step 2: asserting one block containing the new comment_id, got: ${JSON.stringify(result)}`
  );
...
test('TC-MD-COMMENTPARSER-005: mergeComments replaces the block when comment.updated changed', () => {
  console.log(
    '[TC-MD-COMMENTPARSER-005] step 1: building existing blocks from an older version of the comment'
  );
  const oldContent = formatComment(makeComment(), '2026-09-01T00:00:00Z');
  const existing = parseCommentBlocks(oldContent);
  console.log(
    '[TC-MD-COMMENTPARSER-005] step 2: merging a fresh comment with a newer updated timestamp and new body'
  );
  ...
  console.log(
    `[TC-MD-COMMENTPARSER-005] step 3: asserting block was replaced with new body/downloaded_at, got: ${JSON.stringify(result)}`
  );
  assert.equal(result.length, 1);
...
test('TC-MD-COMMENTPARSER-006: mergeComments keeps the block unchanged when comment.updated matches stored', () => {
  console.log(
    '[TC-MD-COMMENTPARSER-006] step 1: building existing blocks from the comment at its original downloaded_at'
  );
  const oldContent = formatComment(makeComment(), '2026-09-01T00:00:00Z');
  const existing = parseCommentBlocks(oldContent);
  console.log(
    '[TC-MD-COMMENTPARSER-006] step 2: merging the same comment again (unchanged updated timestamp)'
  );
  const result = mergeComments(existing, [makeComment()], DOWNLOADED_AT);
  console.log(
    `[TC-MD-COMMENTPARSER-006] step 3: asserting the original block text is preserved byte-for-byte, got: ${JSON.stringify(result)}`
  );
  assert.equal(result.length, 1);
...
test('TC-MD-COMMENTPARSER-007: mergeComments marks a missing comment with a deleted_at marker', () => {
  ...
  const existing = parseCommentBlocks(oldContent);
  console.log(
    '[TC-MD-COMMENTPARSER-007] step 2: merging with an empty fresh-comments list (comment now deleted upstream)'
  );
  const result = mergeComments(existing, [], DOWNLOADED_AT);
  console.log(
    `[TC-MD-COMMENTPARSER-007] step 3: asserting a deleted_at marker was appended, got: ${JSON.stringify(result)}`
  );
  assert.equal(result.length, 1);
  assert.match(result[0], /<!-- comment_id: 10088 -->/);
...
test('TC-MD-COMMENTPARSER-008: mergeComments does not duplicate the deleted_at marker across repeated syncs', () => {
  console.log(
    '[TC-MD-COMMENTPARSER-008] step 1: building existing blocks already marked deleted from a prior sync'
  );
  const oldContent = formatComment(makeComment(), '2026-09-01T00:00:00Z');
  const firstResult = mergeComments(parseCommentBlocks(oldContent), [], '2026-09-01T12:00:00Z');
  const existing = parseCommentBlocks(firstResult.join('\n'));
  console.log(
    '[TC-MD-COMMENTPARSER-008] step 2: merging again with the comment still missing (second sync after deletion)'
  );
  const result = mergeComments(existing, [], DOWNLOADED_AT);
  console.log(
    `[TC-MD-COMMENTPARSER-008] step 3: asserting exactly one deleted_at marker remains, got: ${JSON.stringify(result)}`
  );
  assert.equal(result.length, 1);
  const deletedAtMatches = result[0].match(/<!-- deleted_at:/g) ?? [];
  assert.equal(deletedAtMatches.length, 1);
  assert.match(result[0], /<!-- deleted_at: 2026-09-01T12:00:00Z -->/);
});
