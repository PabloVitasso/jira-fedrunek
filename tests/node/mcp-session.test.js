...
test('TC-MCP-SESSION-001: connect() builds client/transport via injected factories and calls client.connect(transport)', async () => {
  console.log('[TC-MCP-SESSION-001] step 1: wiring a fake client capturing the transport it was connected with');
  let capturedTransport;
  const transport = { marker: 'fake-transport' };
  const client = makeFakeClient({ connect: async (t) => { capturedTransport = t; } });
  console.log('[TC-MCP-SESSION-001] step 2: calling connect()');
  const { session } = makeSession({ client, transport });
  await session.connect();
  console.log(`[TC-MCP-SESSION-001] step 3: asserting the injected transport was passed through, got: ${JSON.stringify(capturedTransport)}`);
...
test('TC-MCP-SESSION-002: callTool() calls client.callTool with { name, arguments: { cloudId, ...args } }', async () => {
  ...
  const client = makeFakeClient({
    callTool: async (request) => { captured = request; return { content: [] }; },
  });
  const { session } = makeSession({ cloudId: 'cloud-42', client });
  await session.connect();
  console.log('[TC-MCP-SESSION-002] step 2: calling callTool("searchJiraIssuesUsingJql", { jql: "key = X" })');
  await session.callTool('searchJiraIssuesUsingJql', { jql: 'key = X' });
  console.log(`[TC-MCP-SESSION-002] step 3: asserting cloudId is auto-injected alongside caller args, got: ${JSON.stringify(captured)}`);
...
test('TC-MCP-SESSION-003: callTool() returns the raw client.callTool result', async () => {
  ...
  await session.connect();
  console.log('[TC-MCP-SESSION-003] step 2: calling callTool and asserting the result is passed through unchanged');
...
test('TC-MCP-SESSION-004: close() calls client.close() and swallows a rejected close (e.g. transport DOMException)', async () => {
  console.log('[TC-MCP-SESSION-004] step 1: wiring a fake client whose close() rejects, mirroring mcp-remote\'s known close()-time DOMException');
  let closeCalled = false;
  const client = makeFakeClient({
    close: async () => { closeCalled = true; throw new DOMException('closed'); },
  });
  const { session } = makeSession({ client });
  await session.connect();
  ...
  console.log(`[TC-MCP-SESSION-004] step 3: asserting client.close() was actually invoked, closeCalled=${closeCalled}`);
...
test('TC-MCP-SESSION-006: callTool() retries after a failed attempt and returns the eventual success', async (t) => {
  console.log('[TC-MCP-SESSION-006] step 1: wiring a fake client whose callTool rejects once then succeeds, with fake timers to skip the retry backoff');
  ...
  const { session } = makeSession({ client });
  await session.connect();
  console.log('[TC-MCP-SESSION-006] step 2: calling callTool and advancing fake timers through the retry backoff');
  ...
  console.log(`[TC-MCP-SESSION-006] step 3: asserting two attempts were made and the successful result was returned, got attempts=${attempts}, result=${JSON.stringify(result)}`);
...
test('TC-MCP-SESSION-007: callTool() rejects once every attempt fails, having exhausted RETRY_COUNT retries', async (t) => {
  console.log('[TC-MCP-SESSION-007] step 1: wiring a fake client whose callTool always rejects, with fake timers to skip the retry backoff');
  t.mock.timers.enable({ apis: ['setTimeout'] });
  let attempts = 0;
  const client = makeFakeClient({
    callTool: async () => {
  ...
  const { session } = makeSession({ client });
  await session.connect();
  console.log('[TC-MCP-SESSION-007] step 2: calling callTool and advancing fake timers through every retry backoff');
  const resultPromise = session.callTool('getJiraIssue', { issueIdOrKey: 'X-1' });
  resultPromise.catch(() => {});
  ...
  console.log(`[TC-MCP-SESSION-007] step 3: asserting the call rejects after exhausting retries, got attempts=${attempts}`);
...
test('TC-MCP-SESSION-008: connect() rejects when the client never resolves connect() within AUTH_TIMEOUT_S', async (t) => {
  console.log('[TC-MCP-SESSION-008] step 1: wiring a fake client whose connect() never resolves, with fake timers to skip the real 90s wait');
  t.mock.timers.enable({ apis: ['setTimeout'] });
  const client = makeFakeClient({ connect: () => new Promise(() => {}) });
  const { session } = makeSession({ client });
  console.log('[TC-MCP-SESSION-008] step 2: calling connect() and advancing fake timers past AUTH_TIMEOUT_S (90s)');
  const connectPromise = session.connect();
  connectPromise.catch(() => {});
  await flushMicrotasks();
  t.mock.timers.tick(90_000);
  console.log('[TC-MCP-SESSION-008] step 3: asserting connect() rejects with the authorization-timeout message');
...
test('TC-MCP-SESSION-005: close() is idempotent — a second call does not re-invoke client.close()', async () => {
  ...
  let closeCount = 0;
  const client = makeFakeClient({ close: async () => { closeCount++; } });
  const { session } = makeSession({ client });
  await session.connect();
  console.log('[TC-MCP-SESSION-005] step 2: calling close() twice');
  await session.close();
  await session.close();
  console.log(`[TC-MCP-SESSION-005] step 3: asserting client.close() was only invoked once, got closeCount=${closeCount}`);
