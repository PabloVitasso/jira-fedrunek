...
test('TC-CLI-DISPATCH-001: "login" command connects and closes McpSession, reporting success', async () => {
  console.log(
    '[TC-CLI-DISPATCH-001] step 1: wiring a fake McpSession recording connect/close calls'
  );
  let connected = false;
  let closed = false;
  ...
  console.log(
    `[TC-CLI-DISPATCH-001] step 3: asserting connect/close were called and status "logged_in", got: ${JSON.stringify(result)}, connected=${connected}, closed=${closed}`
  );
...
test('TC-CLI-DISPATCH-002: "sync" command runs SyncEngine.syncAll with the given issue keys', async () => {
  ...
  console.log(
    '[TC-CLI-DISPATCH-002] step 2: calling dispatch("sync", ["PROJ-1", "PROJ-2"], { syncEngine })'
  );
...
test('TC-CLI-DISPATCH-003: an unknown command rejects rather than silently doing nothing', async () => {
  console.log(
    '[TC-CLI-DISPATCH-003] step 1: calling dispatch("bogus", [], { ... }) and asserting it rejects'
  );
...
test('TC-CLI-DISPATCH-004: "sync" with no issue keys loads tracked_keys from TrackedKeysConfig', async () => {
  ...
  console.log(
    '[TC-CLI-DISPATCH-004] step 2: calling dispatch("sync", [], { syncEngine, trackedKeysConfig })'
  );
...
test('TC-CLI-DISPATCH-005: "track" command adds the given keys via TrackedKeysConfig.add and reports the updated list', async () => {
  ...
  console.log(
    '[TC-CLI-DISPATCH-005] step 2: calling dispatch("track", ["PROJ-3"], { trackedKeysConfig })'
  );
...
test('TC-CLI-DISPATCH-006: "confluence page <id>" dispatches to ConfluenceSyncEngine.syncPage', async () => {
  ...
  console.log(
    '[TC-CLI-DISPATCH-006] step 2: calling dispatch("confluence", ["page", "100000001"], { confluenceSyncEngine })'
  );
  const result = await dispatch(
    'confluence',
    ['page', '100000001'],
    baseDeps({ confluenceSyncEngine })
  );
  console.log(
...
test('TC-CLI-DISPATCH-007: "confluence dir <id>" resolves a label from watchDirs when present', async () => {
  ...
  console.log(
    '[TC-CLI-DISPATCH-007] step 2: calling dispatch("confluence", ["dir", "100000002"], { confluenceSyncEngine, watchDirs })'
  );
...
test('TC-CLI-DISPATCH-007B: "confluence dir <id>" falls back to using the id as its own label when not found in watchDirs', async () => {
  ...
  console.log(
    '[TC-CLI-DISPATCH-007B] step 2: calling dispatch("confluence", ["dir", "999"], { confluenceSyncEngine, watchDirs: [] })'
  );
...
test('TC-CLI-DISPATCH-008: "confluence dirs" dispatches to ConfluenceSyncEngine.syncDirs with the configured watchDirs', async () => {
  ...
  console.log(
    '[TC-CLI-DISPATCH-008] step 2: calling dispatch("confluence", ["dirs"], { confluenceSyncEngine, watchDirs })'
  );
...
test('TC-CLI-DISPATCH-009: "confluence sync" dispatches to ConfluenceSyncEngine.syncSpaces with the configured spaceKeys', async () => {
  ...
  console.log(
    '[TC-CLI-DISPATCH-009] step 2: calling dispatch("confluence", ["sync"], { confluenceSyncEngine, spaceKeys: ["ARCHDOCS"] })'
  );
  await dispatch(
    'confluence',
    ['sync'],
    baseDeps({ confluenceSyncEngine, spaceKeys: ['ARCHDOCS'] })
  );
  console.log(
...
test('TC-CLI-DISPATCH-009B: "confluence pages" dispatches to ConfluenceSyncEngine.syncPages with the configured watchPages', async () => {
  ...
  console.log(
    '[TC-CLI-DISPATCH-009B] step 2: calling dispatch("confluence", ["pages"], { confluenceSyncEngine, watchPages })'
  );
...
test('TC-CLI-DISPATCH-010: an unknown confluence verb rejects rather than silently doing nothing', async () => {
  console.log(
    '[TC-CLI-DISPATCH-010] step 1: calling dispatch("confluence", ["bogus"], { ... }) and asserting it rejects'
  );
  await assert.rejects(
    () => dispatch('confluence', ['bogus'], baseDeps()),
    /unknown confluence verb: bogus/
  );
});
