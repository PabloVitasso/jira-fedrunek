...
test('TC-SYNC-STATE-001: load() on a missing file starts with empty issues/confluence sections', () => {
  ...
  state.load();
  console.log('[TC-SYNC-STATE-001] step 3: asserting getIssue()/getPage() return undefined for any key');
...
test('TC-SYNC-STATE-003: setPage() then getPage() round-trips per-page metadata in memory', () => {
  ...
  state.setPage('100000001', { lastModified: '2026-09-01T00:00:00Z', path: 'ARCHDOCS/100000001-title.md', title: 'Title' });
...
test('TC-SYNC-STATE-004: save() then a fresh load() round-trips both sections to disk', () => {
  console.log('[TC-SYNC-STATE-004] step 1: creating a SyncState, loading, and setting one issue + one page');
  ...
  state.save();
  console.log('[TC-SYNC-STATE-004] step 3: creating a second SyncState instance over the same path and loading it');
  ...
  assert.deepEqual(reloaded.getIssue('PROJ-1'), { issue_updated_at: '2026-09-01T00:00:00Z', comment_ids: [] });
...
test('TC-SYNC-STATE-005B: load() fills in empty issues/confluence sections independently when either key is missing from otherwise-valid JSON', () => {
  ...
  const statePath = makeTempPath();
  fs.writeFileSync(statePath, JSON.stringify({ issues: { 'PROJ-1': { issue_updated_at: 'x', comment_ids: [] } } }), 'utf8');
  const state = new SyncState(statePath);
  console.log('[TC-SYNC-STATE-005B] step 2: calling load() and asserting confluence defaults to {} while issues is preserved');
  state.load();
  ...
  fs.writeFileSync(statePath2, JSON.stringify({ confluence: { 1: { lastModified: 'x', path: 'p.md', title: 'T' } } }), 'utf8');
  const state2 = new SyncState(statePath2);
  console.log('[TC-SYNC-STATE-005B] step 4: calling load() and asserting issues defaults to {} while confluence is preserved');
  state2.load();
...
test('TC-SYNC-STATE-006B: allPages() returns [pageId, meta] entries from the confluence section', () => {
  ...
  console.log('[TC-SYNC-STATE-006B] step 2: calling allPages() and asserting both entries are present');
