...
test('TC-JIRA-CLIENT-001: getIssue calls searchJiraIssuesUsingJql with key = <key>, default fields, and returns the single matched issue', async () => {
  ...
  const fakeIssue = {
    key: 'PROJ-1',
    fields: { summary: 'x', updated: '2026-01-01T00:00:00.000Z' },
  };
  ...
  console.log(
    `[TC-JIRA-CLIENT-001] step 3: asserting the request shape, got: ${JSON.stringify(captured)}`
  );
  ...
  console.log(
    `[TC-JIRA-CLIENT-001] step 4: asserting the matched issue is returned, got: ${JSON.stringify(issue)}`
  );
...
test('TC-JIRA-CLIENT-002: getIssue throws when zero issues match (key not found)', async () => {
  console.log(
    '[TC-JIRA-CLIENT-002] step 1: wiring a fake mcpSession returning an empty issues array'
  );
...
test('TC-JIRA-CLIENT-003: getIssue throws when more than one issue matches key = X (invariant violation)', async () => {
  console.log(
    '[TC-JIRA-CLIENT-003] step 1: wiring a fake mcpSession returning two issues for a single-key lookup'
  );
  const callTool = async () =>
    textResult({ issues: [{ key: 'PROJ-1' }, { key: 'PROJ-1' }], isLast: true });
...
test('TC-JIRA-CLIENT-004: getIssue reads comments inline from fields.comment.comments (no separate call)', async () => {
  console.log(
    '[TC-JIRA-CLIENT-004] step 1: wiring a fake mcpSession returning an issue with inline comments'
  );
  const fakeIssue = {
    key: 'PROJ-1',
    fields: {
  ...
  const issue = await client.getIssue('PROJ-1');
  console.log(
    `[TC-JIRA-CLIENT-004] step 3: asserting fields.comment.comments is present on the returned issue, got: ${JSON.stringify(issue.fields.comment)}`
  );
  assert.deepEqual(issue.fields.comment.comments, [{ id: '1' }, { id: '2' }]);
});

test('TC-JIRA-CLIENT-006: getIssue rejects a key containing JQL-breaking content instead of interpolating it unsanitized', async () => {
  console.log(
    '[TC-JIRA-CLIENT-006] step 1: wiring a fake mcpSession that should never be called for a malformed key'
  );
  let called = false;
  const callTool = async () => {
    called = true;
    return textResult({ issues: [], isLast: true });
  };
  const client = makeClient({ callTool });
  console.log(
    '[TC-JIRA-CLIENT-006] step 2: calling getIssue with a key embedding JQL injection content'
  );
  await assert.rejects(
    () => client.getIssue('PROJ-1" OR key="PROJ-2'),
    /invalid (Jira )?issue key/i
  );
  console.log(
    `[TC-JIRA-CLIENT-006] step 3: asserting callTool was never reached, called=${called}`
  );
...
test('TC-JIRA-CLIENT-005: constructor accepts a custom fields list overriding DEFAULT_JIRA_FIELDS', async () => {
  console.log(
    '[TC-JIRA-CLIENT-005] step 1: wiring a fake mcpSession capturing the requested fields'
  );
  let captured;
  const callTool = async (name, args) => {
  ...
  console.log(
    '[TC-JIRA-CLIENT-005] step 2: constructing JiraClient with a narrower fields list and calling getIssue'
  );
  const client = makeClient({ callTool, fields: ['summary', 'status'] });
  await client.getIssue('PROJ-1');
  console.log(
    `[TC-JIRA-CLIENT-005] step 3: asserting the custom fields list was sent, got: ${JSON.stringify(captured)}`
  );
